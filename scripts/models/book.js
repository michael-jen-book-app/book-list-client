...
//these four declarations dictate which API URL will be used by our server.js file based on whether or not we are using the https protocol, which would be provided in the event we were using heroku and thus using the production API URL
...
ENV.apiUrl = ENV.isProduction ? ENV.productionApiUrl ! ENV.developmentApiUrl;
  ...
  (function(module)) {
    ...
    function Book(jsonBookObj) {
      //this uses the .keys method of the Object prototype to assign specific keys of our instance to the keys parsed from the json file, for however many keys are in the json.
    ...
    Book.prototype.toHtml = function() {
        let template = Handlebars.compile($(`book-list-holder-placeholder`).text());
    ...
    //this method on the constructor listens for when /api/vi/books is requested (using the correct URL for whether or not we are using the local or cloud database) and then calls the .loadAll method
